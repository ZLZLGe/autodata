...
WHEEL_ROOT="/mnt/shared-storage-user/gezhilong/dataharness/dependencies/ms-swift-4.5.2-py311-torch2.7-cu128-v3/wheelhouse"
TEMP_ROOT="$(mktemp -d /tmp/autodata-stage4a-train.XXXXXX)"
...
finish() {
  ...
  trap - EXIT INT TERM
  case "$TEMP_ROOT" in
    /tmp/autodata-stage4a-train.*) rm -rf -- "$TEMP_ROOT" ;;
...
python3 -m autodata_stage4a.worker prepare-train "$AUTODATA_STAGE4A_REQUEST"
...
CONFIG_PATH="$(dirname -- "$AUTODATA_STAGE4A_REQUEST")/train-config.json"
...
TRAIN_EXIT=$?
set -e
export AUTODATA_STAGE4A_TRAIN_EXIT="$TRAIN_EXIT"
unset NPROC_PER_NODE MASTER_PORT
python3 -m autodata_stage4a.worker validate-train "$AUTODATA_STAGE4A_REQUEST"
