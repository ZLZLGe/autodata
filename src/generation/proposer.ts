...
function proposalPrompt(request: GenerationDraftRequest): string {
  ...
  return [
    ...
    'The DataPlugin runtime contract is exact: run(input, context) receives input as a readonly DataSelection array. Every input[i] is {record,provenance}; input[i].record is a CanonicalTrajectory with {schema_version,source,messages,tools}, and its stable ID is input[i].record.source.record_id. The fixed structural fixture and formal source use this same shape. Do not search wrapper objects or read a top-level record_id/id from input[i].',
    'The flat entries in the Host-provided proposal context source_pool.records are planning summaries only; they are not the runtime run input. A minimal valid identity selection is input.map(item => ({ record_id: item.record.source.record_id })).',
    'run must deterministically return an array of {record_id,note?} decisions selecting, filtering, or ordering only input[i].record.source.record_id values. If input.length is nonzero it must return at least one decision. It may derive tool-use signals from input[i].record.messages and input[i].record.tools, but must not generate or modify records, register tools, access files/network/environment/time/randomness, retain mutable state, or depend on anything outside its arguments.',
